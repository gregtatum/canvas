...
describe("QuadMesh", () => {
  ...
  it("extrudes an edge", () => {
    ...
    const leftEdge: Edge = [a, b];
    const bottomEdge: Edge = [b, c];
    const rightEdge: Edge = [c, d];
    const topEdge: Edge = [d, a];
    ...
    Quads.extrudeEdge(mesh, 0, leftEdge, 0.5);
    assertArt(
      mesh,
      "z",
      `
    ...
    Quads.extrudeEdge(mesh, 0, bottomEdge, 0.5);
    assertArt(
      mesh,
      "z",
      `
    ...
    Quads.extrudeEdge(mesh, 0, rightEdge, 1);
    assertArt(
      mesh,
      "z",
      `
    ...
    Quads.extrudeEdge(mesh, 0, topEdge, 1);
    assertArt(
      mesh,
      "z",
      `
    ...
    // Move the center quad to verify everything is attached correctly.
    for (const p of Quads.getPositions(mesh, 0)) {
      p[1] -= 1;
    }
    assertArt(
  ...
  it("extrudes an edge reversed", () => {
    const { mesh } = Quads.createQuad({ w: 2, h: 4, facing: "z+" });
    const [a, b, c, d] = [0, 1, 2, 3];
    const leftEdge: Edge = [b, a];
    const bottomEdge: Edge = [c, b];
    const rightEdge: Edge = [d, c];
    const topEdge: Edge = [a, d];
  ...
  it("returns an edge when extruding an edge", () => {
    const { mesh } = Quads.createQuad({ w: 2, h: 4, facing: "z+" });
    const leftEdge: Edge = [0, 1];
    const newEdge = Quads.extrudeEdge(mesh, 0, leftEdge, 1);
    assertArt(
      mesh,
      "z",
      `
    ...
    mesh.positions[newEdge[0]][1] += 2;
    mesh.positions[newEdge[1]][1] += 2;
  ...
  it("computes normals when extruding an edge", () => {
    ...
    const leftEdge: Edge = [0, 1];
    Quads.extrudeEdge(mesh, 0, leftEdge, 1);
    expect(mesh.normals).toEqual([
      [0, 0, -1],
      [0, 0, -1],
      [0, 0, -1],
